...
const modelRestAPI = require('./modelRestAPI/index');
const modelAuthAPI = require('./modelRestAPI/auth/index');
const loadFilesApi = require('./modelRestAPI/actions/file/index');
...
module.exports = function (Framework, callback) {
  ...
  if (Framework.config.api && Framework.config.api.auth && _.isPlainObject(Framework.config.api.auth) ) {
    authConfig = Framework.config.api.auth;
  }else if (Framework.config.api && Framework.config.api.auth === true) {
    authConfig = {
      model: 'User',
      secret: '36986b3b46d63874e5f6',
      upload: true,
    };
  }
    ...
    async.each(models, function (model, cb) {
      if ( /\.js$/.test(model) ){
        ...
        if (modelConfig){
          ...
          */
          if (modelConfig.pre){
            _.forEach(modelConfig.pre, function (handler, name) {
              if (
          ...
          */
          if (modelConfig.post){
            _.forEach(modelConfig.post, function (handler, name) {
              if (
                ['init', 'validate', 'save', 'remove'].indexOf(name) !== -1
                &&
                _.isFunction(handler)
          ...
          */
          if (modelConfig.methods){
            _.forEach(modelConfig.methods, function (handler, name) {
              if (
                invalidMethods.indexOf(name) === -1
                &&
                _.isFunction(handler)
          ...
          */
          if (modelConfig.queryHelpers){
            _.forEach(modelConfig.queryHelpers, function (handler, name) {
              if (
                typeof name === 'string'
                &&
                _.isFunction(handler)
          ...
          */
          ...
          if (Framework.config.api && Framework.config.api.enable){
            ...
            if (authConfig && authConfig.model === modelName){
  ...
  loadFilesApi(Framework, authConfig);
